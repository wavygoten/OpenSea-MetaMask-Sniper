...
chrome.storage.local.get(["x2y2"], async (res: any) => {
  if (res.x2y2) {
    await x2y2();
  }
});
...
async function x2y2() {
  if (document.location.href.includes("https://x2y2.io/collection")) {
    contentData = [];
    var dom_observer = new MutationObserver(async function (mutation) {
      contentData = [];
    });
            ...
            if (
              ...
              for (let item in contentData) {
                ...
                if (contentData[item]?.tokenid === temptoken) {
                  ...
                  rarityContainer.appendChild(rarityRank);
                  rarityContainer.appendChild(rarityPercent);
                  content?.children[0]?.children[1]?.prepend(rarityContainer);
                }
              }
            }
          });
...
event.on("clickedScan", async () => {
  await toast(
    true,
    "Scanning... Please wait 5-10 minutes and refresh the page."
  );
  let scanCollection = contractData;
  chrome.runtime.sendMessage({ scanCollection });
  console.log(scanCollection);
});
...
function scanButtonAppend() {
  if (document.location.href.includes("https://opensea.io/collection")) {
    if (
      document
        ?.querySelectorAll(".fresnel-container")[5]
        ?.getElementsByClassName("traitsurfer-scan-item").length === 0
    ) {
      const scanButton: HTMLButtonElement = document.createElement("button");
      scanButton.className = "traitsurfer-scan-item";
      scanSvg(scanButton);
      tippy(scanButton, {
        content: "Scan Collection on TraitSurfer",
      });

      scanButton.onclick = async () => {
        // send req to api from background script, use a special variable to check currently fetching in api
        return event.emit("clickedScan");
      };
      document
        .querySelectorAll(".fresnel-container")[5]
        ?.children[0]?.children[0]?.prepend(scanButton);
    }
  }
}
      ...
      button.onclick = async () => {
        ...
        tokenId = document.location.href.includes("opensea.io/activity")
          ? element
              ...
              ?.children[0]?.getAttribute("href")
              ?.split("/")[4]
          : element?.querySelector("a")?.getAttribute("href")?.split("/")[4];
  ...
  if (document.location.href.includes("https://opensea.io/assets")) {
    if (selectors) {
      if (
        ...
        button.onclick = async () => {
          assetId = document.location.href?.split("/")[5];
          tokenId = document.location.href?.split("/")[6];
    ...
    if (
      ...
      contractData = document.location.href?.split("/")[5];
      if (contentData.length === 0) {
        chrome.runtime.sendMessage({ getContractData: true });
      }
      fetch(
        `https://traitsurfer.app/api/${document.location.href?.split("/")[5]}/${
...
function scanSvg(btn: HTMLButtonElement) {
  ...
  const secondPath: SVGElement = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "path"
  );
  ...
  // svg.setAttribute("fill", "#E5E8EB");
  secondPath.setAttribute("fill", "none");
  secondPath.setAttribute("d", "M0 0h24v24H0z");
  path.setAttribute(
    "d",
  ...
  path.setAttribute("fill", "rgba(229,232,235,1)");
  svg.appendChild(secondPath);
  svg.appendChild(path);
  btn.appendChild(svg);
}

async function cartSvg(btn: HTMLButtonElement) {
  const svg: SVGElement = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "svg"
  );
